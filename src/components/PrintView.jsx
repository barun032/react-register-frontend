    ...
    <>
      <style>{`
          @media print {
            ...
            /* White Header */
            th { background-color: white !important; font-weight: bold; text-align: center; color: black !important; }
          ...
          <table className="w-full border-collapse border border-black text-xs text-black font-serif">
            <thead>
              <tr className="border-b-0">
                <th colSpan={totalColumns} className="p-4 border-black align-middle">
                  
                  <div className="flex items-center justify-between w-full pb-2 mb-1">
                    
                    <span className="text-sm font-bold w-32 text-left">
                      Year: {currentYear}
                    </span>

                    <h3 className="text-xl font-bold uppercase text-center flex-grow">
                      Register of Letters {selectedRegister?.replace(' Register', '')}
                      {selectedRegister === registerTypes.RECEIVE && selectedPart && (
                        <span className="ml-2 border border-black px-2 py-0.5 rounded text-lg">{selectedPart}</span>
                      )}
                    </h3>
              ...
              {tableHeaders.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {row.map((header, headerIndex) => (
                    <th key={headerIndex} rowSpan={header.rowspan || 1} colSpan={header.colspan || 1} className="border border-black px-2 py-1 text-center font-bold text-black uppercase align-middle bg-white">
                      {header.name}
                    </th>
                  ))}
                </tr>
